...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _get_html(self):
        ...
        result["html"] = self.env.ref(
            "contabilidad_cfdi.report_trial_balance"
        ).render(rcontext)
        return result
    ...
    def _compute_account_amount(
        ...
        for tb in tb_initial_acc:
            ...
            if acc_id not in total_amount.keys():
                ...
                if foreign_currency:
                    total_amount[acc_id]["initial_currency_balance"] = round(tb["amount_currency"], 2)
                    total_amount[acc_id]["ending_currency_balance"] = round(tb["amount_currency"], 2)
            else:
                if self.env["account.account"].browse(acc_id).cuenta_tipo == 'A' or self.env["account.account"].browse(acc_id).group_id.cuenta_tipo == 'A':
                    total_amount[acc_id]["initial_balance"] = tb["balance"] * -1
        ...
        for group_id in group_ids:
            if groups_data[group_id]["parent_id"]:
                ...
                if nw_id in groups_data.keys():
                    ...
                    if foreign_currency:
                        groups_data[nw_id]["initial_currency_balance"] += groups_data[group_id]["initial_currency_balance"]
                        groups_data[nw_id]["ending_currency_balance"] += groups_data[group_id]["ending_currency_balance"]
                else:
                    groups_data[nw_id] = {}
                    groups_data[nw_id]["initial_balance"] = groups_data[group_id]["initial_balance"]
                    groups_data[nw_id]["debit"] = groups_data[group_id]["debit"]
