...
class CatalogoCuentasReport(models.AbstractModel):
    ...
    def _get_html(self):
        ...
        result["html"] = self.env.ref(
            "contabilidad_cfdi.report_catalogo_cuentas"
        ).render(rcontext)
        return result
    ...
    def _get_accounts_data(self, account_ids):
        ...
        cuentas_de_orden = self.env['trial.balance.report.wizard.cfdi'].browse(ctx.get('active_id')).cuentas_de_orden
        if cuentas_de_orden == True:
            accounts = accounts.filtered(lambda a: a.code[:1] < '8')
        accounts_data = {}
    ...
    def _get_report_values(self, docids, data):
        ...
        if not show_partner_details:
            ...
            if hierarchy_on == "relation":
                ...
                catalogo_cuentas = list(groups_data.values())
                catalogo_cuentas += list(accounts_data.values())
                catalogo_cuentas = sorted(catalogo_cuentas, key=lambda k: k["complete_code"])
                for trial in catalogo_cuentas:
            ...
            if hierarchy_on == "computed":
                ...
                catalogo_cuentas = list(groups_data.values())
                catalogo_cuentas += list(accounts_data.values())
                catalogo_cuentas = sorted(catalogo_cuentas, key=lambda k: k["code"])
        ...
        return {
            ...
            "docs": self.env["trial.balance.report.wizard.cfdi"].browse(wizard_id),
            "foreign_currency": data["foreign_currency"],
