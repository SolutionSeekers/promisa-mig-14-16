...
class GenerarXmlHirarchyWizard(models.TransientModel):
    ...
    fecha_ano = fields.Selection([('2018', '2018'), ('2019', '2019'),('2020', '2020'),('2021', '2021')],
    ...
    def default_get(self, fields_list):
        ...
        if ctx.get('default_fecha_mes'):
            try:
                res['fecha_mes'] = str(ctx.get('default_fecha_mes'))
            except Exception:
                pass
        if ctx.get('default_fecha_ano'):
            try:
                res['fecha_ano'] = str(ctx.get('default_fecha_ano'))
            except Exception:
                pass
        return res
    ...
    def action_validate_xml(self):
        ...
        #accounts = self.env['account.account'].browse(list(accounts_data.keys()))
        values = self.to_json(trial_balance) #total_amount)
    ...
    def to_json(self, total_amount):
        ...
        if self.tipo_de_reporte == 'Catalogo de cuentas':
            request_params.update({'informacion':{
                      ...
                      'fecha_mes': '{:02d}'.format(int(self.fecha_mes)),
                      'fecha_ano': self.fecha_ano,
                      'procesa_nivel': self.procesa_nivel,
                       },})
            for account_id in total_amount:
                account_lines.append({'subctade': account_id['subctade'],
