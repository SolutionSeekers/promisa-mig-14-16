...
class PayslipBatches(models.Model):
    ...
    def export_report_xlsx(self):
        ...
        for col in all_col_list:
            ...
            col_nm += 1
        for t in ['Total Efectivo', 'Total Especie', 'Impuesto 2%', 'Provisión aguinaldo', 'Provisión prima antigüedad']:
...
#                        if amt:
                ...
                #adicionales
                code_col += 1
                worksheet.write(row, code_col, slip.get_amount_from_rule_code('TPER') * 0.02, text_right)
                code_col += 1
                worksheet.write(row, code_col, (work_day*15/365)*slip.employee_id.contract_id.sueldo_diario, text_right)
                code_col += 1
                if slip.employee_id.contract_id.antiguedad_anos >= 12:
                   worksheet.write(row, code_col, (12*slip.employee_id.contract_id.tablas_cfdi_id.salario_minimo)/53*2, text_right)
                else:
                   worksheet.write(row, code_col, 0, text_right)
                row += 1
