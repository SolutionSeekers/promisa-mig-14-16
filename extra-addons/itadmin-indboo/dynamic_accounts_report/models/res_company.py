...
class ins_account_financial_report(models.Model):
    ...
    def _get_children_by_order(self, strict_range):
        ...
        if not strict_range:
            res -= self.env.ref('dynamic_accounts_report.ins_account_financial_report_unallocated_earnings0')
            res -= self.env.ref('dynamic_accounts_report.ins_account_financial_report_equitysum0')
        return res
