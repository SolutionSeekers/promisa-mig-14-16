...
class InsFinancialReport(models.TransientModel):
    ...
    def _compute_account_balance(self, accounts, report):
        ...
        if accounts:
            if self.account_report_id != \
                        self.env.ref('dynamic_accounts_report.ins_account_financial_report_cash_flow0') and self.strict_range:
    ...
    def _compute_report_balance(self, reports):
        ...
        for report in reports:
            ...
            if report.type == 'accounts':
                ...
                if self.account_report_id != \
                        self.env.ref('dynamic_accounts_report.ins_account_financial_report_cash_flow0'):
                    ...
                    for key, value in res2.items():
                        if report in [self.env.ref('dynamic_accounts_report.ins_cash_in_operation_1'),
                                        self.env.ref('dynamic_accounts_report.ins_cash_in_investing_1'),
                                      self.env.ref('dynamic_accounts_report.ins_cash_in_financial_1')]:
                ...
                if self.account_report_id != \
                    ...
                    if report == self.env.ref('dynamic_accounts_report.ins_account_financial_report_cash_flow0'):
                        accounts = self.env['account.account'].search([('company_id','=', self.env['res.company'].search([('id','=',1)]).id),
    ...
    def get_account_lines(self, data):
        ...
        for report in child_reports:
            ...
            vals = {
                ...
                'level': report.level,
                'company_currency_id': self.env['res.company'].search([('id','=',1)]).currency_id.id,
    ...
    def get_report_values(self):
        ...
        company_domain = [('company_id', '=', self.env['res.company'].search([('id','=',1)]).id)]
        ...
        analytic_tags = self.env['account.analytic.tag'].sudo().search(
            ['|', ('company_id', '=', self.env['res.company'].search([('id','=',1)]).id), ('company_id', '=', False)])
        ...
        used_context['company_id'] = self.env['res.company'].search([('id','=',1)]).id
        ...
        comparison_context['company_id'] = self.env['res.company'].search([('id','=',1)]).id
        ...
        data['currency'] = self.env['res.company'].search([('id','=',1)]).currency_id.id
        ...
        if self.account_report_id == \
            ...
            data['form']['rtype'] = 'CASH'
                ...
                self.env.ref('dynamic_accounts_report.ins_account_financial_report_profitandloss0'):
    ...
    def _get_default_report_id(self):
        ...
        return self.env.ref('dynamic_accounts_report.ins_account_financial_report_profitandloss0').id

    @api.model
    def _get_default_date_range(self):

        mycomp=self.env['res.company'].search([('id','=',1)])

        return mycomp.date_range
    ...
    financial_year = fields.Selection(
        ...
        string='Financial Year', default=lambda self: self.env['res.company'].search([('id','=',1)]).financial_year, required=True)
    ...
    view_format = fields.Selection([
        ...
        default='vertical',
        string="Format")
    company_id = fields.Many2one('res.company', string='Company', required=True, default=lambda self: self.env['res.company'].search([('id','=',1)]))
    strict_range = fields.Boolean(
        string='Strict Range',
        default=lambda self: self.env['res.company'].search([('id','=',1)]).strict_range)
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            'dynamic_accounts_report.ins_financial_report_pdf').report_action(self,
                                                                     data)
