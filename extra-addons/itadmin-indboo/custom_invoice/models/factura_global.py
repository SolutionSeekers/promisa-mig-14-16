...
class Facturaglobal(models.Model):
    ...
    def init(self):
        ...
        for company in company_id:
            faltas_sequence = self.env['ir.sequence'].search([('code', '=', 'factura.global'), ('company_id', '=', company.id)])
    ...
    def create(self, vals):
        if vals.get('number', _('Draft Invoice')) == _('Draft Invoice'):
            if 'company_id' in vals:
                vals['number'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('factura.global') or _('Draft Invoice')
            else:
