...
class CajaAhorro(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('caja.nomina') or _('New')
            else:
    ...
    def action_cancelar(self):
        if self.state == 'draft':
           ...
           if datetime.today() > datetime.strptime(self.fecha_aplicacion, "%Y-%m-%d"):
