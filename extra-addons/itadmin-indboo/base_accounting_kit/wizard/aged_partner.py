...
class AccountAgedTrialBalance(models.TransientModel):
    ...
    journal_ids = fields.Many2many('account.journal', string='Diarios',
                                   required=True)
    period_length = fields.Integer(string='Longitud del periodo (días)',
    ...
    def _print_report(self, data):
        ...
        if period_length <= 0:
            raise UserError(_('Debe elegir un periodo mayor a 0.'))
        if not data['form']['date_from']:
            raise UserError(_('Debe poner una fecha inicial.'))
